...
import { arr2menu, formatMenuPath } from '../../utils/menu'
...
const AsideMenu: FC = memo(() => {
  ...
  useEffect(() => {
    ...
    const type = {
      [IUserType.company]: 'company',
      [IUserType.readyCompany]: 'readyCompany',
      [IUserType.systemAdmin]: 'systemAdmin',
      [IUserType.couponAdmin]: 'couponAdmin',
      
    }

    console.log((type as any)[userType]);

    
    

    fetchAsideMenus((type as any)[userType]).then(res => {
      const menus = arr2menu(res)
      console.log(menus)
  ...
  function renderChildrenMenu(navList: IAsideMenu[]): ReactNode {
    ...
    return navList.map(nav => {
      ...
      if (children?.length) {
        return (
          <SubMenu title={title} icon={icon} key={path}>
    ...
    <Layout.Sider {...getSliderProps()}>
      <ContainerAsideInner>
        <div className="info">
          ...
          <h3 className="info__title">优惠券发放系统</h3>
        </div>
